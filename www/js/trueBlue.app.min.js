...
trueBlue.run(function($timeout, $location, $rootScope, $localStorage, $filter, AppService, DataService, UserService, ShoppingService, AnalyticsService, LanguageService, Override) {
  ...
  $rootScope.APIpath = (server === "live") ? "http://live.trueblue.guru/application/"+APIversion+"/public" : "http://localhost.trueblue.guru/api/"+APIversion+"/public";
  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      if ($rootScope.editProduct) {
        scope.edit = true;
        scope.cartProduct = angular.copy($rootScope.editProduct);
        scope.product = scope.cartProduct.data;
      }
      else {
        scope.product = $rootScope.tempProduct;
      }

      console.log("addToCart", scope.edit, scope.cartProduct, scope.product);
      ...
      // add a variable item
      scope.addVariableItem = function($tpv, $limit) {
        console.log($tpv, $limit);
        if (!$tpv.selected) {
          $tpv.selected = [];
        }
        $tpv.selected.push($tpv.items[0]);
      ...
      scope.removeVariableItem = function($tpvSelected, $tpvs) {
        var index = $tpvSelected.indexOf($tpvs);
        $tpvSelected.splice(index, 1);
      ...
      scope.evaluateTotal = function() {
        console.log("evaluateTotal addToCart", scope.product);
        var selectedVariation = scope.product.selectedVariation;
        scope.subtotal = parseFloat(selectedVariation.value);
        console.log("variation", selectedVariation.value, scope.subtotal);

        for (var a = 0; a < Object.keys(scope.product.variables).length; a++) {
          var variable = scope.product.variables[a];
          // console.log("variable", variable, variable.selected, variable.selected.length, Object.keys(variable.selected).length);
          if (variable.selected && Object.keys(variable.selected).length > 0) {
            for (var b = 0; b < Object.keys(variable.selected).length; b++) {
              var selectedVariable = variable.selected[b];
              if (selectedVariable.type == 0) {
                scope.subtotal += parseFloat(selectedVariable.value);
                // console.log("variable $", selectedVariable, a, b, selectedVariable.value, scope.subtotal);
              } else if (selectedVariable.type == 1) {
                var $p = scope.subtotal * (parseFloat(selectedVariable.value)/100);
                scope.subtotal += $p;
                // console.log("variable %", selectedVariable, a, b, selectedVariable.value, scope.subtotal);
              }
            }
      ...
      for (var v in scope.product.variables) {
        ...
        var var_id = scope.product.variables[v].id;
        if (scope.cartProduct && scope.cartProduct.variable && scope.cartProduct.variable[var_id]) {
          var cv = scope.cartProduct.variable[var_id];
          if (!scope.product.variables[v].selected) {
            scope.product.variables[v].selected = [];
          }
          for (var w in cv) {
            // console.log(var_id, cv[w]);
            scope.product.variables[v].selected[w] = findVariableItem(var_id, cv[w]);
          }
        } else if (scope.product.variables[v].required_flag === 1) {
          if (!scope.product.variables[v].selected) {
            scope.product.variables[v].selected = [];
          }
          scope.product.variables[v].selected.push(scope.product.variables[v].items[0]);
        }
        // console.log("scope.product.variables", v, scope.product.variables[v]);
      }

      if (scope.cartProduct && scope.cartProduct.variation) {
        scope.product.selectedVariation = findVariationItem(scope.cartProduct.variation);
      }
      else {
        scope.product.selectedVariation = scope.product.variations[0];
      }
      // end setup default

      function findVariationItem(cvar_id) {
        for (var va in scope.product.variations) {
          if (scope.product.variations[va].id == cvar_id) {
            return scope.product.variations[va];
            break;
          }
        }
      }
      ...
      scope.removeCart = function() {
        ...
        // console.log("id", $rootScope.editProduct.id);
        // console.log("variable", $rootScope.editProduct.variable);
        // console.log("variation", $rootScope.editProduct.variation);

        if ($rootScope.cart['products']) {
          for (var a = 0; a < $rootScope.cart['products'].length; a++) {
            var product = $rootScope.cart['products'][a];
            if (product.id === $rootScope.editProduct.id) {
              if (!product.variation && !$rootScope.editProduct.variation || product.variation === $rootScope.editProduct.variation) {
                // console.log("found?", $rootScope.cart['products'][a], $rootScope.editProduct);
                // console.log("product.variable.length", product.variable.length, "$rootScope.editProduct.variable.length", $rootScope.editProduct.variable.length, !product.variable.length && !$rootScope.editProduct.variable.length);
                // console.log("product.variable", JSON.stringify(product.variable), "$rootScope.editProduct.variable", JSON.stringify($rootScope.editProduct.variable), JSON.stringify(product.variable) === JSON.stringify($rootScope.editProduct.variable));
                // console.log(product.variable);
                // console.log($rootScope.editProduct.variable);
                // console.log(Object.keys(product.variable).length);
                // console.log(Object.keys($rootScope.editProduct.variable).length);
                // console.log(JSON.stringify(product.variable));
                // console.log(JSON.stringify($rootScope.editProduct.variable));
                if (!product.variable && !$rootScope.editProduct.variable
                  || (product.variable && !Object.keys(product.variable).length) && ($rootScope.editProduct.variable && !Object.keys($rootScope.editProduct.variable).length)
                  || JSON.stringify(product.variable) === JSON.stringify($rootScope.editProduct.variable)) {
                  $rootScope.cart['products'].splice(a, 1);
                }
              }
            }
          }
        ...
        $localStorage.cart = JSON.stringify($rootScope.cart['products']);
      ...
      scope.updateCart = function() {
        ...
        var $product = {};
        $product['id'] = scope.product.id;
        $product['variation'] = scope.product.selectedVariation.id;//$('form .variation.selected',element).attr('v_id');
        //console.log($('form .variable[set_id] select:visible',element).length);
        if (scope.product.variables) {
          $product['variable'] = {};
          for (var a = 0; a < Object.keys(scope.product.variables).length; a++) {
            var variable = scope.product.variables[a];
            if (variable.selected && Object.keys(variable.selected).length > 0) {
              if (!$product['variable'][variable.set_id]) {
                $product['variable'][variable.set_id] = [];
              }
              for (var b = 0; b < Object.keys(variable.selected).length; b++) {
                var selectedVariable = variable.selected[b];
                $product['variable'][variable.set_id].push(selectedVariable.id);
              }
            }
          }
        }
        ...
        // console.log("CART", $cart);
        // console.log("NEW CART PRODUCT", $new_product);
        // console.log("ORIGINAL CART PRODUCT", $original);
        // console.log("includes?", $cart.includes($original));
        // $localStorage.cart = $cart.replace($original, $new_product);
        // console.log("Updated $localStorage.cart", $localStorage.cart);

        // console.log("index", scope.cartProduct.index);
        // console.log("old product", $rootScope.cart['products'][scope.cartProduct.index]);
        // console.log("new product", $product);
        $rootScope.cart['products'][scope.cartProduct.index] = $product;
        // console.log("$rootScope.cart after", $rootScope.cart);
        $localStorage.cart = JSON.stringify($rootScope.cart["products"]);
        // console.log("$localStorage.cart after", $localStorage.cart);
    ...
    link: function(scope, element, attrs) {
      ...
      DataService.resolveClient().then(function(data) {
        if ($localStorage.cart) {
          ...
          console.log("existing cart", JSON.parse($localStorage.cart));
        }
      }, function(e) {
        //console.log("error", e);
      });
              ...
              if ($rootScope.cart && $rootScope.cart['products']) {
                ...
                if (scope.cartProducts) {
                  DataService.getProductDetailsCart(scope.cartProducts).then(function(data) {
                    ...
                    scope.cartProducts = angular.copy(data);
      ...
      scope.editProduct = function($index) {
        console.log("editProduct", $index, scope.cartProducts[$index]);
        ...
        if (product.type && product.type == "reward") {
          ...
          $rootScope.editProduct.index = $index;
      ...
      scope.evaluateTotal = function() {
        console.log("evaluateTotal shoppingCart", scope.total);
        ...
        if ($rootScope.userDetails && $rootScope.userDetails.vip) {
          // console.log("$rootScope.userDetails.vip", $rootScope.userDetails.vip);
          var discount = $rootScope.userDetails.vip.discount;
          // console.log("discount", discount, "scope.total", scope.total);
          $p = parseFloat(scope.total) * (parseFloat(discount)/100);
          // console.log("$p", $p);
          scope.pre_discount = scope.total;
          scope.vip_discount = $p;
          $rootScope.cart['vip_discount'] = $p;
          scope.total -= parseFloat($p);
        }
...
trueBlue.directive('paymentDetails', ['DataService', '$localStorage', '$rootScope', '$timeout', '$filter', 'UserService', 'ShoppingService', 'AnalyticsService', function(DataService, $localStorage, $rootScope, $timeout, $filter, UserService, ShoppingService, AnalyticsService) {
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      scope.submitMonerisPayment = function() {
        //console.log("submitPayment");
        if (!form) {
          form = $('form', element);
        }
        ...
        var cart_user_id = ($rootScope.userDetails) ? $rootScope.userDetails.id : $rootScope.cart['phone'];
        var exp_year = $('select[name="year"]',form).val();

        obj["client_id"] = $rootScope.clientId;
        obj["user_id"] = cart_user_id;
        obj["location_id"] = $rootScope.cart['location_id'];
        obj["order_id"] = $rootScope.cart['order_id'];
        obj["amount"] = scope.total;
        obj["name"] = $('input[name="name"]',form).val();
        obj["number"] = $('input[name="number"]',form).val();
        obj["exp_month"] = $('select[name="month"]',form).val();
        obj["exp_year"] = String(exp_year[2]) + String(exp_year[3]);
        obj["cvc"] = $('input[name="cvc"]',form).val();
        scope.monerisCharge(obj);
      }

      scope.monerisCharge = function(obj) {
        ShoppingService.chargeMoneris(obj).then(function(data) {
          //console.log(obj, data);
          if (!data.error) {
            // charged!
            //console.log("DONE CHARGING!", obj);
            AnalyticsService.registerEvent("moneris", "charge", "success-"+$rootScope.cart['order_id']);
            status = "sent";
            // it will always be paid coming from this service.
            var paid = 1;
            // apply discount and update order to reflect new totals
            ...
            form.removeClass("processing");
            //console.log("error charging the stripe customer :(");
            AnalyticsService.registerEvent("moneris", "charge", "error: "+data.message);
          }
        }, function(e) {
          //console.log("error", e);
        });
...
function unregisterPush() {
  ...
  } else if (typeof window.FirebasePlugin !== "undefined") {
  ...
  injector.invoke(function (PushService) {
    ...
    if (typeof PushNotification !== "undefined") {
      PushNotification.hasPermission(function(permit) {
        ...
        PushService.registerPushID($token, platform, isAllowed)
      });
    } else if (typeof window.FirebasePlugin !== "undefined") {
      // alert("gets here");
  ...
  if (typeof PushNotification !== "undefined") {
    ...
    push.on('registration', function(data) {
      ...
      if (data.registrationId) {
        console.log("on registration and token is:" + data.registrationId);
        registerPush(data.registrationId);
      }else {
        console.log("no id, data is: " + data);
      }
    });
    ...
    window.FirebasePlugin.getToken(function(token) {
      console.log("registering firebase", token);
      // alert("firebase: " + token);
      // $('[js-fcm]').val(token);
      registerPush(token);
    }, function(error) {
      console.log("error registering firebase token", token);
    });

    window.FirebasePlugin.onTokenRefresh(function(token) {
      registerPush(token);
    }, function(error) {
      console.log("error refreshing firebase token", token);
    });

    window.FirebasePlugin.onNotificationOpen(function(notification) {
      // alert("onNotificationOpen: "+notification);
      injector.invoke(function (PushService) {
        PushService.onNotification(notification);
      });
    }, function(error) {
      // alert("onNotificationOpen error: "+error);
    });
  } else {
    console.log("PushNotification", typeof PushNotification);
    console.log("window.FirebasePlugin", typeof window.FirebasePlugin);
  }
}
      ...
      //console.log("getUpdateList DATA", response.data);
          ...
          var clientUpdating = false;
            ...
            for ($p in checkThese) {
              ...
              // console.log(checkThese[$p], getData.data.updates[0][checkThese[$p]], response.data.updates[0][checkThese[$p]]);
              if ($rootScope.server === 'dev'
                || getData.data.updates == undefined
                || getData.data.updates[0] == undefined
                || getData.data.updates[0][checkThese[$p]] == false
                || getData.data.updates[0][checkThese[$p]] == undefined
                || response.data.updates[0][checkThese[$p]] == null
                || response.data.updates[0][checkThese[$p]] == false
                || response.data.updates[0][checkThese[$p]] == undefined
                || getData.data.updates[0][checkThese[$p]] != response.data.updates[0][checkThese[$p]]) {
                // DOWNLOAD THE UPDATE!
                if (checkThese[$p] == "client") {
                  ...
                  // alert("Add to Download list: "+checkThese[$p]);
                  downloadThese.push(checkThese[$p]);
                }
              }
            }
...
trueBlue.service('ShoppingService', ['$http', '$q', '$timeout', '$localStorage', '$filter', '$rootScope', 'AnalyticsService', 'DataService', function($http, $q, $timeout, $localStorage, $filter, $rootScope, AnalyticsService, DataService) {
  ...
  this.addOrder = function($cart) {
    ...
    dataString.push({ name: "payment", value: $rootScope.clientInfo.stripe_flag || $rootScope.clientInfo.moneris_flag });
    ...
    $http.post(
      ...
      $submit.removeClass('processing');
        ...
        if ($rootScope.clientInfo.stripe_flag == 1 || $rootScope.clientInfo.moneris_flag == 1) {
  ...
  // MONERIS STUFF
  this.chargeMoneris = function(obj) {
    var deferred = $q.defer();
    var url = $rootScope.APIpath + "/moneris/charge";
...
trueBlue.service('LanguageService', ['$rootScope', '$timeout', '$localStorage', 'AnalyticsService', function($rootScope, $timeout, $localStorage, AnalyticsService) {
  ...
  this.text["ENG"] = {
    ...
    monerisPowered: "Payments processed by Moneris",
...
trueBlue.controller('productDetailsController', function($rootScope, $scope, $routeParams, $timeout, $filter, $localStorage, DataService, UserService, AnalyticsService) {
  ...
  $scope.addtocart = function() {
    ...
    if (!$noOptions) {
      $rootScope.tempProduct = angular.copy($scope.product);
      if ($scope.product.image) {
        var a = $scope.product.image;
        var b = "100px_";
        var position = a.lastIndexOf('/')+1;
  ...
  $scope.addOrder = function() {
    ...
    ShoppingService.addOrder($rootScope.cart).then(function(data) {
      ...
      if (!data.error) {
        ...
        } else if ($rootScope.clientInfo.moneris_flag == 1) {
          $rootScope.cart['order_id'] = data.order_id;
          $scope.gotoPaymentDetails();
        } else {
...
trueBlue.controller('vipController', function($rootScope, $scope, UserService, DataService) {
  $rootScope.currentParent = "vips";
  $scope.vips = $rootScope.vips;
  // console.log("VIPS", $scope.vips);

  $scope.$watch(function() {
    return $rootScope.userDetails;
  }, function() {
    if ($rootScope.userDetails) {
      for ($vip in $scope.vips) {
        var $points_needed = $scope.vips[$vip].minimum_quantity - $rootScope.userDetails.points;
        $scope.vips[$vip].points_needed = $points_needed;
        $scope.vips[$vip].selected = ($rootScope.userDetails.vip && $rootScope.userDetails.vip.vip_id == $scope.vips[$vip].id) ? true : false;
      }
    }
  });
});
