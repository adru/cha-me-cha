  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      scope.evaluateTotal = function() {
        ...
        for (var a = 0; a < scope.product.variables.length; a++) {
          var variable = scope.product.variables[a];
          if (variable.selected && variable.selected.length > 0) {
            for (var b = 0; b < variable.selected.length; b++) {
          ...
          for (var a = 0; a < $rootScope.cart['products'].length; a++) {
            ...
            if (product.id === $rootScope.editProduct.id) {
              if (!product.variation && !$rootScope.editProduct.variation || product.variation === $rootScope.editProduct.variation) {
                ...
                if (!product.variable.length && !$rootScope.editProduct.variable.length || JSON.stringify(product.variable) === JSON.stringify($rootScope.editProduct.variable)) {
  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      scope.editProduct = function($index) {
        console.log("editProduct", scope.cartProducts[$index]);
